...
- (void)setBarUI{
    ...
    [self xw_addObserverBlockForKeyPath:@"money" block:^(id obj, id oldVal, id newVal) {
        NSLog(@"kvo，修改name为%@", newVal);
        self.allMoneyLabel.text = [NSString stringWithFormat:@"总计￥:%.2f",[newVal floatValue]];
        if ([newVal floatValue] == 0) {
            self.selectAllButton.selected = NO;
        }
        self.balanceButton.enabled = [newVal floatValue] > 0 ?YES:NO;
        self.deleteButton.enabled = [newVal floatValue] > 0 ?YES:NO;
    }];
    
    [self xw_addObserverBlockForKeyPath:@"isNormalState" block:^(id obj, id oldVal, id newVal) {
        BOOL isNormal =  [newVal boolValue];
